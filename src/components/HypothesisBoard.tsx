  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h2 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.01em' }}>{tr.hypothesesHeader}</h2>
      ...
      {showCreate && (
        <div className="card animate-fade-up" style={{ padding: 16, marginBottom: 12 }}>
          <p style={{ fontSize: 13, fontWeight: 600, marginBottom: 12, color: 'var(--text)' }}>{tr.newHypothesis}</p>
          <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <input type="text" name="title" placeholder={tr.hypothesisTitlePlaceholder} required className="input" />
            <textarea name="description" placeholder={tr.hypothesisDescPlaceholder} rows={3} required className="input" style={{ height: 'auto', padding: '8px 12px' }} />
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button type="button" onClick={() => setShowCreate(false)} className="btn-secondary" style={{ height: 32, fontSize: 12 }}>{tr.cancel}</button>
            </div>
          </form>
        </div>
      )}
      ...
      {hypotheses.length === 0 && !showCreate && (
        <div className="card" style={{ padding: 40, textAlign: 'center', marginBottom: 16 }}>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>{tr.noHypothesesYet}</p>
          <button onClick={() => setShowCreate(true)} className="btn-secondary" style={{ height: 32, fontSize: 12 }}>{tr.createFirstHypothesis}</button>
        </div>
      )}
            ...
            <div key={h.id} className="card" style={{ padding: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                <Link href={`/hypothesis/${h.id}`}>
                  <button className="btn-secondary" style={{ height: 28, fontSize: 11 }}>{tr.open}</button>
                </Link>
                <button onClick={() => handleImprove(h.id)} disabled={improvingId === h.id} className="btn-ghost" style={{ height: 28, fontSize: 11 }}>
                  {improvingId === h.id
                    ? <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 10, height: 10, border: '1.5px solid #D1D5DB', borderTopColor: '#6B7280', borderRadius: '50%', animation: 'spin 0.8s linear infinite', display: 'inline-block' }} />{tr.improving}</span>
                    : tr.improveWithAi}
                </button>
              </div>
              ...
              {improveResult?.id === h.id && (
                <div className="ai-block animate-fade-up" style={{ marginTop: 12, padding: 12 }}>
                  <p style={{ fontSize: 11, fontWeight: 600, color: '#1A1A1A', marginBottom: 10 }}>{tr.aiSuggestionsLabel}</p>
                  <div className="card" style={{ padding: 10, marginBottom: 8 }}>
                    ...
                    <button onClick={() => handleApply(h.id, improveResult.result.improvedTitle, improveResult.result.improvedDescription)} className="btn-primary" style={{ height: 26, fontSize: 11 }}>{tr.apply}</button>
                  </div>
                  {improveResult.result.variations?.map((v, i) => (
                    <div key={i} className="card" style={{ padding: 10, marginBottom: 8 }}>
                      <p style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>{tr.variation(i + 1)}</p>
                      ...
                      <button onClick={() => handleApply(h.id, v.title, v.description)} className="btn-secondary" style={{ height: 26, fontSize: 11 }}>{tr.useThis}</button>
                    </div>
                  ))}
                  <button onClick={() => setImproveResult(null)} className="btn-ghost" style={{ height: 26, fontSize: 11 }}>{tr.keepOriginal}</button>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="card" style={{ padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div>
            <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 2 }}>{tr.aiSuggestionsTitle}</p>
          ...
          <button onClick={handleGenerateAI} disabled={loadingAI} className="btn-secondary" style={{ height: 32, fontSize: 12 }}>
            {loadingAI ? <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 12, height: 12, border: '1.5px solid #D1D5DB', borderTopColor: '#6B7280', borderRadius: '50%', animation: 'spin 0.8s linear infinite', display: 'inline-block' }} />{tr.generating}</span> : tr.generateIdeas}
          </button>
        </div>
